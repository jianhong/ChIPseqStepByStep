...
library(ChIPpeakAnno)
packagePath <- system.file("extdata", package = "ChIPpeakAnno")
ex.broadPeaks <- dir(packagePath, "broadPeak")
ex.peaks <- sapply(ex.broadPeaks, function(.ele){
    toGRanges(file.path(packagePath, .ele), format="broadPeak")
})
ex.peaks
ex.overlaps <- findOverlapsOfPeaks(ex.peaks)
names(ex.overlaps)
names(ex.overlaps$peaklist)
makeVennDiagram(ex.overlaps)
```
</div>
...
library(ChIPpeakAnno)
packagePath <- system.file("extdata", package = "ChIPpeakAnno")
ex.peaks <- toGRanges(file.path(packagePath, "YY1.broadPeak"), format="broadPeak")
library(TxDb.Hsapiens.UCSC.hg19.knownGene)
annoData <- toGRanges(TxDb.Hsapiens.UCSC.hg19.knownGene)
ex.peaks.anno <- annotatePeakInBatch(ex.peaks, AnnotationData = annoData)
ex.peaks.anno
```
</div>
...
## browse the tracks
browseTracks(trackList(ex.scores, trs), gr=ex.gr)
```
</div>
